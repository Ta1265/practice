...
The ocean is to the right of the buildings. A building has an ocean view if
 the building can see the ocean without obstructions. Formally, a
...
function findBuildings(heights) {
  ...
  for (let i = heights.length - 1; i >= 0; i -= 1) {
    ans.push(j);
    j = i - 1;
    while (j >= 0 && heights[j] <= heights[i]) j -= 1;
    i = j + 1;
  }
  return ans.reverse();
}
...
(function () {
  console.log(findBuildings([4, 2, 3, 1]), 'expecting [0,2,3]');
  console.log(findBuildings([2, 2, 2, 2]), 'expecting [0,2,3]');
  console.log(findBuildings([4, 3, 2, 1]), 'expecting [0,2,3]');
  console.log(findBuildings([1, 2, 3, 4]), 'expecting [0,2,3]');
  console.log(findBuildings([]), 'expecting [0,2,3]');
}());
