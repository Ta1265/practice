...
function generateSampleView(users) {
    ...
    for (var i = 0; i < users.length; i++) {
        if (users[i].id % 2 === 0) {
            var output = users[i].address.street + ', ' + users[i].address.suite + ', ' +
                users[i].address.city + ', ' + users[i].address.zipcode
            ans.push(output);
        } else {
            ans.push(users[i].email);
        }
    }
    return ans;
}
...
expected = ["Sincere@april.biz", "Victor Plains, Suite 879, Wisokyburgh, 90566-7771"]
actual = generateSampleView(users);

assertArraysEqual(actual, expected, 'Taylors test');